...
function checkEmail(em) {
    const reg = /^[a-zA-z]+\W?[a-zA-z]+@[a-zA-Z\.]+\.[a-z]{2,3}$/;
...
console.log(checkEmail('mail-name@mail.ua')); // true
console.log(checkEmail('mail-name@mail.com.ua')); // true!!!!!!!!!!!false
 ...
 */
console.log('------------------')
function checkDomainUrl(dom) {
    const reg = /^(http:|https:)\/\/(?!:\/\/)([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,5}?$/;
 ...
 */
function createLinksFromDomains(dom) {
    const reg  = new RegExp('(http:|https:)\\/\\/(?!:\\/\\/)([a-zA-Z0-9-_]+\\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\\.[a-zA-Z]{2,5}', 'gi');
dom.match(reg).forEach((el) =>  dom = dom.replace(el, `<a href="${el}" target="_blank">${el.replace(/^(http:|https:)\/\//gi, '')}</a>`));
// console.log(dom)
    return dom ;
}
